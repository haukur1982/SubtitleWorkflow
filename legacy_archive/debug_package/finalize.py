...
OUTBOX = BASE_DIR / "4_FINAL_OUTPUT"
...
def _run_watcher():
    print(f"👀 Typesetter Active. Waiting for JSONs in {INBOX}...")
    while True:
        system_health.update_heartbeat("finalize")
        # 1. Check for APPROVED files (VIP - High Priority)
        for file in INBOX.glob("*_APPROVED.json"):
            stem = file.name.replace("_APPROVED.json", "")
            normalized_path = OUTBOX / f"{stem}_normalized.json"
            if not (OUTBOX / f"{stem}.srt").exists() or not normalized_path.exists():
                json_to_srt(file)

        # 2. Check for ICELANDIC files (Auto Pilot)
        for file in INBOX.glob("*_ICELANDIC.json"):
            # Skip if an APPROVED version exists (avoids double processing)
            stem = file.name.replace("_ICELANDIC.json", "")
            if (INBOX / f"{stem}_APPROVED.json").exists():
                continue
            ...
            try:
                ...
                if isinstance(content, dict) and content.get("needs_human_review", False):
                    # VIP File waiting for review - SKIP
                    continue
                
                # If we get here, it's either Auto-Pilot or Old Format -> PROCESS
                normalized_path = OUTBOX / f"{stem}_normalized.json"
                if not (OUTBOX / f"{stem}.srt").exists() or not normalized_path.exists():
                    json_to_srt(file)
